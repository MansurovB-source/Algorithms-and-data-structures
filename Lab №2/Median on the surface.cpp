...
double k(point p, int x, int y) {
    return (p.y - y) / (double) (p.x - x);
}


vector<point> merge(vector<point> points, int l1, int r1, int l2, int r2) {
...
void merge_sort(vector<point> &points, int left, int right) {
    if (right - left + 1 < 2) {
        return;
    }
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        points[i].k = k(points[i], minX, minY);
    }
